...
async function verifyDocumentWithThirdParty(documentId, documentType) {
...
async function main() {
    try {
        ...
        let admin = await database_1.default.user.findUnique({
            where: { email: 'admin@spana.co.za' }
        });
        if (!admin) {
            admin = await database_1.default.user.create({
                data: {
        ...
        await database_1.default.adminOTP.updateMany({
            where: {
                adminEmail: admin.email,
                otp,
                used: false
        ...
        const adminToken = jsonwebtoken_1.default.sign({ id: admin.id }, process.env.JWT_SECRET || 'your-secret-key', { expiresIn: '5h' });
        ...
        for (let i = 0; i < 4; i++) { // Approve 4 out of 5
            ...
            const updated = await database_1.default.serviceProviderApplication.update({
                where: { id: app.id },
                data: {
                    status: 'approved',
                    reviewedBy: admin.id,
        ...
        for (const app of approvedApplications) {
            ...
            const provider = await database_1.default.serviceProvider.create({
                data: {
                    userId: user.id,
                    skills: app.skills,
        ...
        for (const { provider, user } of registeredProviders) {
            ...
            for (const docType of documentTypes) {
                ...
                await database_1.default.documentVerification.create({
                    data: {
        ...
        const allDocuments = await database_1.default.document.findMany({
        ...
        for (const booking of activeBookings) {
            ...
            if (service) {
                ...
                const allBookings = await database_1.default.booking.findMany({
                    where: {
                        serviceId: { in: (await database_1.default.service.findMany({ where: { providerId: provider.id } })).map(s => s.id) },
                        rating: { not: null }
                    }
                });
        ...
        for (const complaint of complaints) {
            await database_1.default.complaint.update({
        ...
        for (const { provider } of registeredProviders) {
            ...
            if (completedBookings.length > 0) {
                ...
                const payout = await database_1.default.providerPayout.create({
                    data: {
                        providerId: provider.id,
                        periodStart,
                        periodEnd,
        ...
        console.log('');
        const stats = {
            admins: await database_1.default.user.count({ where: { role: 'admin' } }),
            providers: await database_1.default.serviceProvider.count(),
            ...
            payments: await database_1.default.payment.count({ where: { status: 'completed' } }),
            complaints: await database_1.default.complaint.count(),
            resolvedComplaints: await database_1.default.complaint.count({ where: { status: 'resolved' } }),
            payouts: await database_1.default.providerPayout.count()
        };
