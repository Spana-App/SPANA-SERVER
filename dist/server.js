...
try {
    ...
    if (typeof setupMonitoring === 'function')
        setupMonitoring(app);
}
catch (e) {
...
const initSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('call-offer', ({ toUserId, offer, bookingId }) => {
            if (!toUserId || !offer)
                return;
            io.to(toUserId).emit('call-offer', { from: socket.userId, offer, bookingId });
        });
        socket.on('call-answer', ({ toUserId, answer, bookingId }) => {
            if (!toUserId || !answer)
                return;
...
app.use((req, res, next) => {
    ...
    res.on('finish', () => {
        ...
        const methodColor = req.method === 'GET' ? chalk.blue :
        ...
        console.log(chalk.gray(new Date().toISOString()), methodColor(req.method.padEnd(7)), statusColor(res.statusCode), chalk.white(req.originalUrl), chalk.gray(`${duration}ms`), cacheIndicator);
    });
    next();
});
    ...
    try {
        ...
        res.json = (data) => {
            if (res.statusCode >= 200 && res.statusCode < 300) {
                ...
                cacheSet(cacheKey, payload, 'EX', 300).catch((err) => console.error(chalk.red('❌  Cache error:'), err));
            }
...
app.get('/health', async (req, res) => {
    ...
    const healthCheck = {
        ...
        redis: USE_REDIS ? (redisClient && redisClient.connected ? 'connected' : 'disconnected') : 'disabled'
    };
...
app.get('/health/detailed', async (req, res) => {
    ...
    if (USE_REDIS && (!redisClient || !redisClient.connected)) {
            ...
            setTimeout(() => {
                (async () => {
                    try {
                        ...
                        const redisStatus = USE_REDIS ? (redisClient && redisClient.connected ? 'connected' : 'disconnected') : 'disabled';
                        // Check database status (with timeout to not block)
                        let dbState = 'checking...';
                        try {
                            const dbCheck = await Promise.race([
                                database_1.default.$queryRaw `SELECT 1`.then(() => 'connected'),
...
module.exports.redisDel = redisDel;
