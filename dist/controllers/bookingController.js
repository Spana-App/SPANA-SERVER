...
exports.createBooking = async (req, res) => {
    try {
        ...
        if (req.user.role === 'customer' && booking.service.provider) {
        ...
        try {
            ...
            if (io) {
                ...
                if (service && service.provider) {
                    const providerUser = await database_1.default.serviceProvider.findUnique({
...
exports.completeBooking = async (req, res) => {
    try {
        ...
        const finalBooking = await database_1.default.booking.findUnique({
            where: { id: booking.id },
            include: {
                service: {
                    include: {
...
exports.updateLocation = async (req, res) => {
    try {
        ...
        const providerLoc = updatedBooking.providerLiveLocation;
        const customerLoc = updatedBooking.customerLiveLocation;
        ...
        if (providerLoc && customerLoc && providerLoc.coordinates && customerLoc.coordinates) {
            ...
            const proximityUpdate = { distanceApart: distance };
            ...
            if (Object.keys(proximityUpdate).length > 1) {
                const finalUpdated = await database_1.default.booking.update({
                    where: { id: booking.id },
        ...
        const serviceForEmit = await database_1.default.service.findUnique({
            where: { id: booking.serviceId },
            include: { provider: true }
        });
...
exports.getBookingById = async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin') {
            ...
            if (!booking.service.provider || booking.service.provider.userId !== req.user.id) {
                return res.status(403).json({ message: 'Not authorized to view this booking' });
            }
        }
        else {
    ...
    try {
        ...
        if (service && service.provider) {
            ...
            if (serviceProvider) {
                ...
                const providerServices = await database_1.default.service.findMany({
                    where: { providerId: serviceProvider.id },
                    select: { id: true }
                });
                ...
                const totalRatings = bookings.reduce((sum, b) => sum + (b.rating || 0), 0);
...
exports.rateCustomer = async (req, res) => {
    try {
        ...
        if (customer) {
            const customerBookings = await database_1.default.booking.findMany({
                where: {
                    customerId: customer.id,
            ...
            const totalRatings = customerBookings.reduce((sum, b) => sum + (b.customerRating || 0), 0);
...
async function releaseEscrowFunds(paymentId, bookingId) {
    try {
