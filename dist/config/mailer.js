...
async function sendMailWithRetry({ to, subject, text, html, from, attachments, maxRetries = 5 }) {
    const retryDelay = (attempt) => Math.min(2000 * Math.pow(2, attempt), 15000); // Exponential backoff, max 15s
...
async function sendMail({ to, subject, text, html, from, attachments }) {
    ...
    try {
        ...
        const mailOptions = { from: fromAddress, to, subject };
        if (text)
...
async function sendMailWithInlineImages({ to, subject, html, inlineImages, from, text }) {
    // inlineImages: [{ filename, path, cid }]
    const attachments = (inlineImages || []).map((img) => ({ filename: img.filename, path: img.path, cid: img.cid }));
...
function buildVerificationEmail({ firstName, lastName, verificationLink }) {
    const name = [firstName, lastName].filter(Boolean).join(' ').trim() || 'there';
...
async function sendEmailVerification({ to, name, link }) {
    if (USE_EMAIL_SERVICE && emailService.isEmailServiceEnabled()) {
        try {
            return await emailService.sendVerificationEmailViaService({
                to,
...
function buildReceiptEmail({ toRole, amount, currency, bookingId, transactionId, createdAt }) {
...
async function sendReceiptEmail({ to, toRole, amount, currency, bookingId, transactionId, createdAt }) {
...
function buildInvoiceEmail({ name, invoiceNumber, bookingId, serviceTitle, amount, currency, jobSize, basePrice, multiplier, calculatedPrice, tipAmount, date, transactionId }) {
...
async function sendInvoiceEmail({ to, name, invoiceNumber, bookingId, serviceTitle, amount, currency, jobSize, basePrice, multiplier, calculatedPrice, tipAmount, date, transactionId }) {
...
function buildAdminOTPEmail({ name, otp, verificationLink }) {
...
async function sendAdminOTPEmail({ to, name, otp, verificationLink }) {
    if (USE_EMAIL_SERVICE && emailService.isEmailServiceEnabled()) {
        try {
...
function buildAdminCredentialsEmail({ name, email, password }) {
...
async function sendAdminCredentialsEmail({ to, name, email, password }) {
    if (USE_EMAIL_SERVICE && emailService.isEmailServiceEnabled()) {
        try {
...
function buildCustomerWelcomeEmail({ name, email, password }) {
...
async function sendCustomerWelcomeEmail({ to, name, email, password }) {
    if (USE_EMAIL_SERVICE && emailService.isEmailServiceEnabled()) {
        try {
            return await emailService.sendEmailViaService({
                to,
...
module.exports = {
    ...
    async verifySmtp() {
        try {
            ...
            const details = { message: error && error.message ? error.message : String(error) };
            if (error && error.code)
