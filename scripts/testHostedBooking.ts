/**
 * Test Booking Endpoint with Exact Payload Format - HOSTED VERSION
 */
...
async function testHostedBooking() {
  log('\n🧪 Testing Booking Endpoint on HOSTED SERVER\n', colors.blue);
  ...
  try {
    ...
    try {
      ...
      if (healthRes.status === 200) {
        ...
        log(`   ⚠️  Server returned status ${healthRes.status}\n`, colors.yellow);
      }
    } catch (error: any) {
    ...
    const testEmail = `test_hosted_${Date.now()}@test.com`;
    try {
    ...
    try {
      ...
      log(`   ⚠️  Get services failed: ${error.response?.data?.message || error.message}\n`, colors.yellow);
    }
    ...
    try {
      const bookingRes = await axios.post(
        ...
        {
          headers: { Authorization: `Bearer ${customerToken}` },
          timeout: 30000, // Longer timeout for hosted server
      ...
      if (error.response) {
        ...
        log('   ⚠️  Request timed out. Hosted server may be slow to respond.', colors.yellow);
      } else {
        log(`   Error: ${error.message}`, colors.red);
        if (error.code === 'ECONNREFUSED') {
          log('   ⚠️  Server not accessible.', colors.yellow);
        }
      }
    }
    ...
    try {
      ...
      if (res.status === 400) {
        ...
        log(`      Message: ${res.data.message}`, colors.cyan);
      } else {
    ...
    // Test 6.3: Missing both serviceId and serviceTitle
    log('\n   6.3. Missing both serviceId and serviceTitle...', colors.yellow);
    try {
      const res = await axios.post(
        `${API_BASE_URL}/bookings`,
        {
      ...
      if (res.status === 400) {
        log('      ✅ Correctly rejected missing serviceId/serviceTitle', colors.green);
        log(`      Message: ${res.data.message}`, colors.cyan);
      } else {
        log(`      ❌ Expected 400, got ${res.status}`, colors.red);
      }
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
      log('\n⚠️  Cannot connect to hosted server.', colors.yellow);
      log(`   URL: ${API_BASE_URL}`, colors.cyan);
      log('   The server may be starting up or experiencing issues.', colors.yellow);
    }
    process.exit(1);
  }
}
