...
async function main() {
  try {
    ...
    for (const data of providerData) {
      ...
      let provider = await prisma.serviceProvider.findUnique({ where: { userId: user.id } });
      if (!provider) {
        provider = await prisma.serviceProvider.create({
          data: {
            ...
            serviceAreaCenter: {
              type: 'Point',
              coordinates: user.location ? (user.location as any).coordinates : [28.0473, -26.2041]
            }
          }
        });
      } else {
        ...
        if (!existing) {
          await prisma.document.create({
            data: {
              ...
              verified: true,
              verifiedBy: (await prisma.user.findFirst({ where: { role: 'admin' } }))?.id || user.id,
              verifiedAt: new Date(),
    ...
    for (const { provider, user } of providers) {
      ...
      const serviceNames = serviceTemplates[serviceType as keyof typeof serviceTemplates] || ['General Service'];
    ...
    for (const { provider, user } of providers) {
      ...
      const providerBookings = bookings.filter(b => 
        providerServices.some(s => s.id === b.serviceId) && b.status === 'completed' && b.rating
      );
    ...
    const stats = {
      ...
      providers: await prisma.serviceProvider.count({ where: { isProfileComplete: true } }),
      ...
      canceledBookings: await prisma.booking.count({ where: { status: 'cancelled' } }),
      payments: await prisma.payment.count(),
      refundedPayments: await prisma.payment.count({ where: { status: 'refunded' } })
    };
