/**
 * Completes eksnxiweni@gmail.com provider profile and creates test bookings
 * Run: npx ts-node scripts/completeEksnxiweniProfile.ts
 */

import prisma from '../lib/database';

const PROVIDER_EMAIL = 'eksnxiweni@gmail.com';
...
async function completeEksnxiweniProfile() {
  try {
    console.log('🔧 Finding eksnxiweni@gmail.com and completing profile...\n');
    ...
    if (!user) {
      console.error('❌ User not found:', PROVIDER_EMAIL);
      process.exit(1);
    }
    ...
    await prisma.user.update({
      ...
      data: {
        ...
        profileImage:
          user.profileImage ||
          'https://api.dicebear.com/7.x/avataaars/svg?seed=eksnxiweni',
    ...
    if (!provider) {
      const created = await prisma.serviceProvider.create({
        data: {
          userId: user.id,
          skills: ['Plumbing', 'Electrical', 'General Maintenance', 'Appliance Repair'],
          experienceYears: 5,
          isOnline: true,
          rating: 4.5,
          totalReviews: 12,
          ...
          serviceAreaRadius: 30, // 30km radius
          serviceAreaCenter: {
            type: 'Point',
            coordinates: JHB_COORDS,
            address: JHB_ADDRESS,
      ...
      const updated = await prisma.serviceProvider.update({
        ...
        data: {
          skills: provider.skills?.length
            ? provider.skills
            : ['Plumbing', 'Electrical', 'General Maintenance', 'Appliance Repair'],
          experienceYears: provider.experienceYears > 0 ? provider.experienceYears : 5,
          isOnline: true,
          rating: provider.rating > 0 ? provider.rating : 4.5,
          totalReviews: provider.totalReviews || 12,
          isVerified: true,
          isIdentityVerified: true,
          isProfileComplete: true,
          applicationStatus: 'active',
          ...
          serviceAreaRadius: provider.serviceAreaRadius > 0 ? provider.serviceAreaRadius : 30,
          serviceAreaCenter:
      ...
      if (!existing) {
        await prisma.document.create({
          data: {
            ...
            verifiedBy,
            verifiedAt: new Date(),
            metadata: docType === 'id_number' ? { idNumber: '9001015009087' } : null,
          },
        });
      ...
      for (let i = 0; i < Math.min(bookingsToCreate, bookingScenarios.length); i++) {
        ...
        if (scenario.paymentStatusValue !== 'failed') {
          ...
          await prisma.payment.create({
            data: {
              ...
              escrowStatus: scenario.paymentStatusValue === 'paid' ? 'held' : scenario.paymentStatusValue === 'refunded' ? 'refunded' : null,
              commissionRate: 0.15,
        ...
        try {
          ...
          const defaultSteps = [
            ...
            { name: 'Payment Required', status: scenario.paymentStatus === 'pending' ? 'pending' : 'completed' },
            { name: 'Payment Received', status: scenario.paymentStatus === 'paid_to_escrow' ? 'completed' : 'pending' },
            {
    ...
    console.log(`   - Pending: ${finalBookings.filter((b) => b.status === 'pending' || b.requestStatus === 'pending').length}`);
  } catch (error) {
